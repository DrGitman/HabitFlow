...
class Notification:
    @staticmethod
    def create(user_id, type, title, message=None, related_entity_type=None, related_entity_id=None):
        ...
        query = """
            ...
            RETURNING *
        """
        return execute_query(query, (user_id, type, title, message, related_entity_type, related_entity_id), fetch_one=True)

    @staticmethod
    def get_all_for_user(user_id, limit=50):
        """Get all notifications for a user"""
        query = "SELECT * FROM notifications WHERE user_id = %s ORDER BY created_at DESC LIMIT %s"
